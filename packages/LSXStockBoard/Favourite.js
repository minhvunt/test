Stock.Favourite = AV.extend(AV.Module, {
	...
	draw: function()
	{
		return '<div id="category" class="pags bound-category"></div><'+'script>$AV(\'LSXStockBoard.Favourite\').getItems();Stock.updateElementsPosition();<'+'/script>';
	},
	initEvents:function(){
			/*Second box*/				
	...
	itemOnclick:function(){
		var that = this;
		AV.dialog('LSXStockBoard.Favourite.AddItem');
							...
							for (var j = 0, length = Stock.favouriteItems.length; j < length; j++) {
								{
									...
									code+='<li id="favourite-'+sym+'"><div><div style="float:left">'+sym+'</div><span class ="del-favourite " onclick="$AV(\'LSXStockBoard.Favourite\').remove(\''+sym+'\', this.parentNode)">X</span></div><br clear="all" /></li>';
								}
							}
	...
	*/
	remove:function(symbol, obj) {
		$('#lista .choose-symbol').removeClass('firstItem');
		$("#lista #wc"+symbol).removeClass("selected-symbol").addClass('choose-symbol').removeAttr('id').html('<a class="show-menu" href="javascript:$AV(\'LSXStockBoard.Favourite\').itemOnclick();void(0)">'+Lang.selectStock+'</a>');
