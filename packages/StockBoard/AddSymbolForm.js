Stock.AddSymbolForm = AV.extend(AV.Module, {
			...
			AV.load('3rdparty/jQuery/ui/ui.draggable.js', function(){
				...
				$('#add-category-float-form').validate({
					...
					submitHandler: function(form){
						$AV('StockBoard.AddSymbolForm').addListClick($.trim(form.name.value), $.trim(form.symbols.value))
					}
				});
			});
		});
	...
	filterSymbolList: function(floor){
		...
		for(var i in Stock.allSymbols)
		{
			...
			{
				obj.append('<tr onmouseover="Stock.board.trOver(this)" onmouseout="Stock.board.trOut(this)" onclick="$AV(\'StockBoard.AddSymbolForm\').cmd(\'add\', \''+i+'\');$(this).remove();"><td>'+floors[symbol[6]]+'</td><td>'+i+'</td><td>'+symbol[4]+'</td></tr>');
			}
		}
	...
	saveDefaultList: function()
	{
		...
		//AV.alert('Save default list: '+$('#select-list').val()+' (Sua o trong ham /packages/StockBoard/AddSymbolForm.js - saveDefaultList())');
	},
	...
	buy: function(symbol)
	{
		...
		//AV.alert('Buy symbol: '+symbol+'; Price: '+Stock.currentPrice+'; Floor: '+Stock.floor+' (Sua o trong ham /packages/StockBoard/AddSymbolForm.js - buy())');
	},
	sell: function(symbol)
	{
		parent.Order_Buy_QuickOrder('Sell', symbol, Stock.floor);
		//AV.alert('Sell symbol: '+symbol+'; Price: '+Stock.currentPrice+'; Floor: '+Stock.floor+' (Sua o trong ham /packages/StockBoard/AddSymbolForm.js - sell())');
	},
