...
$.widget("ui.multiselect", {
	...
	_updateCount: function() {
		...
		if(this.options.linkTo)
		{
			...
			$(this.options.linkTo).val(options.join(','));
		}
	},
	...
	_applyItemState: function(item, selected) {
		if (selected) {
			var that = this;
			if (this.options.sortable)
				item.children('span').addClass('ui-icon-arrowthick-2-n-s').removeClass('ui-helper-hidden').addClass('ui-icon').click(function(){
					var obj = this.parentNode;
					if(obj.previousSibling)
					{
						$(obj).insertBefore($(obj.previousSibling));
						that.selectedList.find('li').each(function() {
							if ($(this).data('optionLink'))
								$(this).data('optionLink').remove().appendTo(that.element);
						});
