...
static string tickerKey = "";

string _filePath = System.Configuration.ConfigurationSettings.AppSettings["FilePath"];
    ...
    DateTime endDate, int resolution)
{
    ...
    timeStamps = db.getTimeStamps(); //DateTime
    highData = db.getHighData(); //double
    lowData = db.getLowData(); //double
    openData = db.getOpenData(); //double
    closeData = db.getCloseData(); //double
    volData = db.getVolData(); //double
     * */

    System.Data.DataTable dt = CreateFinancialChartData(ticker, startDate, endDate, "_full");
    timeStamps = new DateTime[dt.Rows.Count];
    highData = new double[dt.Rows.Count];
    lowData = new double[dt.Rows.Count];
    closeData = new double[dt.Rows.Count];
    openData = new double[dt.Rows.Count];
    volData = new double[dt.Rows.Count];

    for (int i = 0; i < dt.Rows.Count; i++)
    {
        timeStamps[i] = DateTime.Parse(dt.Rows[i]["Time"].ToString());
        highData[i] = double.Parse(dt.Rows[i]["Highest"].ToString());
        lowData[i] = double.Parse(dt.Rows[i]["Lowest"].ToString());
        closeData[i] = double.Parse(dt.Rows[i]["ClosePrice"].ToString());
        openData[i] = double.Parse(dt.Rows[i]["OpenPrice"].ToString());
        volData[i] = double.Parse(dt.Rows[i]["TotalShare"].ToString());
    }
}

private System.Data.DataTable CreateFinancialChartData(string symbol, DateTime startDate, DateTime endDate, string fileName)
{
    ...
    //Load du lieu
    if (System.IO.File.Exists(String.Format("{0}\\{1}\\{2}{3}.txt", _filePath, symbol, symbol, fileName)))
    {
        ...
        for (int i = lines.Length - 1; i >= 0; i--)
        {
            ...
            if (values.Length > 0)
            {
                ...
                if (dtmDate >= startDate && dtmDate <= endDate)
                {
...
public BaseChart drawChart()
{
    ...
    try { System.Threading.Thread.CurrentThread.CurrentCulture =
          System.Globalization.CultureInfo.InvariantCulture; }
    catch { /* security exception can occur in low trust environment - ignores it */ }
    ...
    string compareKey = "";
    if (Request.QueryString["CompareWith"] == null)
        compareKey = CompareWith.Text.Trim();
    else
        compareKey = Request.QueryString["CompareWith"];
    ...
    if (Request.QueryString["TickerSymbol"] == null)
    {
        if(Request.Form["TickerSymbol"] == null)
        {
            if (Request.QueryString["q"] != null)
            {
                tickerKey = Request.QueryString["q"];
            }
            else
            {
        ...
        {
            tickerKey = Request.Form["TickerSymbol"];
        }   
    }        
    else
    {
        tickerKey = Request.QueryString["TickerSymbol"].ToString().Trim();        
    }

    TickerSymbol.Text = tickerKey;
    ...
    m.addPlotAreaTitle(Chart.BottomRight,
        "<*font=Arial,size=8*>(c) vfpress.vn");
    ...
    <tr valign="top">
        <td style="width:150px; background:#bbddff">
            ...
            <div class="inputtitle">
                ...
                <asp:TextBox runat="server" id="TickerSymbol" class="input" style="width:140px;">HAG</asp:TextBox>
            </div>
            <div class="inputtitle">
                <b>Compare With</b><br />
            ...
            <div class="inputtitle">
                ...
                <asp:DropDownList runat="server" id="Band" class="input" style="width:140px;">
                    ...
                    <asp:ListItem value="DC">Donchain Channel</asp:ListItem>
